...
from accounts.models import BACHELOR_DEGREE, Student
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS("Starting OAuth user profile fix..."))

        # Find OAuth users
        oauth_users = User.objects.filter(socialaccount__isnull=False).distinct()
            ...
            try:
                ...
                if not dry_run:
                    ...
                    if user.is_student and not hasattr(user, "student"):
                        ...
                        self.stdout.write(
                            self.style.SUCCESS(
                                f"Created student profile for {user.email}"
                            )
                        )
        ...
        if dry_run:
            ...
            self.stdout.write(self.style.SUCCESS("OAuth user profile fix completed!"))
