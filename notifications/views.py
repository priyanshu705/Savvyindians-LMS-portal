    ...
    return JsonResponse(
        {
            ...
            "unread_count": Notification.objects.for_user(request.user)
            .unread()
            .count(),
        }
    )
...
def notification_preferences_view(request):
    ...
    if request.method == "POST":
        ...
        preferences.email_progress_updates = (
            request.POST.get("email_progress_updates") == "on"
        )
        preferences.email_course_completion = (
            request.POST.get("email_course_completion") == "on"
        )
        preferences.email_achievements = request.POST.get("email_achievements") == "on"
        preferences.email_announcements = (
            request.POST.get("email_announcements") == "on"
        )
        preferences.email_reminders = request.POST.get("email_reminders") == "on"

        preferences.app_progress_updates = (
            request.POST.get("app_progress_updates") == "on"
        )
        preferences.app_course_completion = (
            request.POST.get("app_course_completion") == "on"
        )
        ...
        if quiet_start:
            preferences.quiet_hours_start = timezone.datetime.strptime(
                quiet_start, "%H:%M"
            ).time()
        if quiet_end:
            preferences.quiet_hours_end = timezone.datetime.strptime(
                quiet_end, "%H:%M"
            ).time()
...
def create_announcement_api(request):
    ...
    try:
        ...
        count = bulk_create_announcement(
            title=title, message=message, users=students, priority=priority
        )
    ...
    return JsonResponse(
        {
            ...
            "notifications": [
                {"id": n.id, "title": n.title, "type": n.notification_type}
