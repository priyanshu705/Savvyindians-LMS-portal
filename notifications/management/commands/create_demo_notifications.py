...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Creating {notifications_count} demo notifications for {users_count} users..."
            )
        )
        ...
        if len(users) < users_count:
            ...
            additional_users = User.objects.exclude(id__in=[u.id for u in users])[
        ...
        for i in range(notifications_count):
            ...
            message = message.replace(
                "{date}", (timezone.now() + timedelta(days=7)).strftime("%B %d, %Y")
            )
            ...
            notification = Notification.objects.create(
                ...
                related_course=(
                    course
                    if notification_type in ["progress_update", "course_completion"]
                    else None
                ),
                related_video=(
                    video if notification_type == "video_completion" else None
                ),
            )
            ...
            if created_count % 10 == 0:
                self.stdout.write(
                    f"Created {created_count}/{notifications_count} notifications..."
                )
        ...
        self.stdout.write(
            f"Read notifications: {total_notifications - unread_notifications}"
        )
