...
class Program(models.Model):
    ...
    title = models.CharField(
        ...
        help_text=_(
            "Name of the bootcamp program (e.g., 'AI & Machine Learning Bootcamp')"
        ),
    )
    ...
    is_featured = models.BooleanField(
        default=False, help_text=_("Show this bootcamp on homepage featured section")
    )
    total_duration_weeks = models.IntegerField(
        default=12, help_text=_("Total bootcamp duration in weeks")
    )
...
class Course(models.Model):
    ...
    def is_current_module(self):
        ...
        try:
            ...
            current_module = semester_module_map.get(
                current_semester.semester, MODULE_ONE
            )
...
def log_save_course(sender, instance, created, **kwargs):
    verb = "created" if created else "updated"
    ActivityLog.objects.create(message=_(f"The course '{instance}' has been {verb}."))


@receiver(post_delete, sender=Course)
def log_delete_course(sender, instance, **kwargs):
...
class UploadVideo(models.Model):
    ...
    video = models.FileField(
        ...
        help_text=_(
            "Valid video formats: mp4, mkv, wmv, 3gp, f4v, avi, mp3 (Optional if YouTube URL is provided)"
        ),
        validators=[
    ...
    video_duration = models.CharField(
        max_length=20, blank=True, help_text=_("Video duration (auto-populated)")
    )
    is_youtube_video = models.BooleanField(
        default=False, help_text=_("Automatically set if YouTube URL is provided")
    )
    ...
    def clean(self):
        ...
        if not self.youtube_url and not self.video:
            raise ValidationError(
                _("Either YouTube URL or video file must be provided")
            )
...
def log_save_uploadvideo(sender, instance, created, **kwargs):
    if created:
        ActivityLog.objects.create(
            message=_(
...
def log_delete_uploadvideo(sender, instance, **kwargs):
    ActivityLog.objects.create(
        message=_(
...
class VideoProgress(models.Model):
    ...
    watch_time = models.PositiveIntegerField(
        default=0, help_text=_("Watch time in seconds")
    )
    total_duration = models.PositiveIntegerField(
        default=0, help_text=_("Total video duration in seconds")
    )
    last_position = models.PositiveIntegerField(
        default=0, help_text=_("Last watched position in seconds")
    )

    # Status fields
    is_completed = models.BooleanField(default=False)
    completion_percentage = models.FloatField(
        default=0.0, help_text=_("Completion percentage (0-100)")
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.total_duration > 0:
            self.completion_percentage = min(
                (self.watch_time / self.total_duration) * 100, 100.0
            )
...
def log_video_progress(sender, instance, created, **kwargs):
    if created:
        ActivityLog.objects.create(
            message=_(
                f"Student '{instance.student.username}' started watching '{instance.video.title}'."
            )
        )
