...
def add_youtube_video():
    ...
    for video_data in videos_to_add:
        try:
            ...
            if not video_data.get("youtube_url") or "YOUR_VIDEO_ID" in video_data.get(
                "youtube_url", ""
            ):
                print(
                    f"⚠️  Skipped: {video_data.get('title', 'Unknown')} - Please update VIDEO_ID"
                )
                error_count += 1
                continue
            ...
            existing = UploadVideo.objects.filter(
                title=video_data["title"], course=course
            ).exists()
            ...
            print("   Create the course first in admin panel")
            print()
            error_count += 1
